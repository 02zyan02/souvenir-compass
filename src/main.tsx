...
function App() {
  ...
  const formatPrice = (product: Product) => {
    if (displayCurrency === 'LOCAL') return product.local_price
    const rates = new Map((catalog?.exchangeRates ?? []).map(rate => [rate.currency_code, Number(rate.rate_per_eur)]))
    const localRate = rates.get(product.currency_code)
    const displayRate = rates.get(displayCurrency)
    if (!localRate || !displayRate) return product.local_price
    const amount = Number(product.price_amount) / localRate * displayRate
    return new Intl.NumberFormat('en', { style: 'currency', currency: displayCurrency, maximumFractionDigits: 2 }).format(amount)
  }
  ...
  return <main>
    ...
    <section id="discover" className="shell content"><div className="section-heading"><div><p className="eyebrow">{labels.eyebrow}</p><h2>{labels.where}</h2></div><button className="text-btn" onClick={() => { setQuery(''); setActiveCategory('All') }}>{labels.all}</button></div>
      ...
      {selectedCountry && <div className="country-guide"><div className="guide-flag">{selectedCountry.flag}</div><div className="guide-copy"><p className="eyebrow">{selectedCountry.city.toUpperCase()} · {labels.field}</p><h3>{zh ? `推荐购买：${selectedCountry.chinese_buy}` : `Buy ${selectedCountry.buy}`}</h3><p>{zh ? selectedCountry.chinese_tip : selectedCountry.tip}</p><div className="quick-picks"><span>{zh ? '购物清单' : 'Quick picks'}</span>{picks.map(product => <b key={product.id}>{zh ? product.chinese_name : product.name}</b>)}<small>⌖ {zh ? '建议地点：' : 'Where: '}{selectedCountry.shopping_location} · <a href={selectedCountry.reference_url} target="_blank" rel="noreferrer">Source</a></small></div></div><button onClick={() => setQuery(selectedCountry.name)}>{labels.see} {zh ? selectedCountry.chinese_name : selectedCountry.name} <span>→</span></button></div>}
      <div className="section-heading finds"><div><p className="eyebrow">{labels.curated}</p><h2>{labels.pack}</h2></div><div className="catalog-controls"><label className="currency-picker">Currency <select value={displayCurrency} onChange={event => setDisplayCurrency(event.target.value)}><option value="LOCAL">Local</option><option value="USD">USD</option><option value="EUR">EUR</option><option value="MYR">MYR</option><option value="SGD">SGD</option></select></label><div className="pills">{['All', ...(catalog?.categories ?? [])].map(category => <button key={category} className={activeCategory === category ? 'active' : ''} onClick={() => setActiveCategory(category)}>{zh ? ({ All: '全部', Snacks: '零食', Beauty: '美妆', Fashion: '时尚', Home: '家居' }[category] ?? category) : category}</button>)}</div></div></div>
      <div className="products">{filtered.map(product => <article className="product" key={product.id} onClick={() => openProduct(product)}><div className="product-art"><img src={product.image_url} alt={zh ? product.chinese_name : product.name} /><label>{product.tag}</label><button onClick={event => { event.stopPropagation(); toggleSaved(product.name) }} aria-label={`Save ${product.name}`} className={saved.includes(product.name) ? 'saved' : ''}>{saved.includes(product.name) ? '♥' : '♡'}</button></div><div className="product-info"><small>{product.country} · {product.category}</small><h3>{zh ? product.chinese_name : product.name}</h3><p>{product.note}</p><div><strong>{formatPrice(product)}</strong><span> ★ {product.rating}</span></div></div></article>)}</div>
    </section>
    ...
    {detailProduct && <div className="modal-backdrop" onClick={() => setDetailProduct(null)}><section className="product-modal" onClick={event => event.stopPropagation()}><button className="close" onClick={() => setDetailProduct(null)}>×</button><div className="product-detail-gallery"><img src={(detailProduct.photos[activePhoto] ?? { image_url: detailProduct.image_url }).image_url} alt={zh ? detailProduct.chinese_name : detailProduct.name} /><div>{detailProduct.photos.map((photo, index) => <button key={photo.id} className={index === activePhoto ? 'active' : ''} onClick={() => setActivePhoto(index)}><img src={photo.image_url} alt={photo.alt_text} /></button>)}</div></div><div className="product-detail-copy"><p className="eyebrow">{detailProduct.country.toUpperCase()} · {detailProduct.category.toUpperCase()}</p><h2>{zh ? detailProduct.chinese_name : detailProduct.name}</h2><p className="product-description">{detailProduct.description}</p><div className="rating">★ {detailProduct.rating} <span>({detailProduct.review_count} reviews)</span></div><dl><div><dt>{displayCurrency === 'LOCAL' ? 'Local price' : `Price in ${displayCurrency}`}</dt><dd>{formatPrice(detailProduct)}</dd></div><div><dt>Local price</dt><dd>{detailProduct.local_price}</dd></div><div><dt>Typical overseas price</dt><dd>{detailProduct.typical_overseas_price}</dd></div><div><dt>Savings</dt><dd>{detailProduct.savings}</dd></div><div><dt>Why buy it</dt><dd>{detailProduct.why_buy}</dd></div><div><dt>Best shops</dt><dd>{detailProduct.best_shops}</dd></div><div><dt>Airport availability</dt><dd>{detailProduct.airport_availability}</dd></div><div><dt>Online availability</dt><dd>{detailProduct.online_availability}</dd></div></dl><div className="reviews"><h3>Reviews</h3>{detailProduct.reviews.map(review => <blockquote key={`${review.reviewer_name}-${review.created_at}`}><b>{review.reviewer_name} · ★ {review.rating}</b><p>{review.body}</p></blockquote>)}</div><a className="source-link" href={detailProduct.reference_url} target="_blank" rel="noreferrer">Read source guide ↗</a></div></section></div>}
  </main>
}
